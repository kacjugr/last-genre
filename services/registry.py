...
spotify.py directly:
    ...
    period_choices[service] -> list[str]
        The selectable period/time-range values for that service, used to
        build index.html's shared 'period' <select>.
...
HOW TO ADD A NEW SERVICE
  ...
  3. At the top level of that file — not inside a function, so it runs at
     import time — import this module and register:
       ...
       period_choices['<service>'] = YOUR_PERIOD_CHOICES
